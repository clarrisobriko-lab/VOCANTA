...
def process_runtime_queue(database: Database, profile, *, pipeline=run_application_pipeline) -> int:
    """Process queued vacancies through the single production application pipeline."""
    ...
    for row in candidates:
        ...
        if result.automation is None:
            ...
            continue
        database.record_queue_audit(row["id"], "ATS_PIPELINE", "ACCEPTED", result.decision.reason)
