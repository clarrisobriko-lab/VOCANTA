...
def ensure_reply_schema(connection) -> None:
    ...
    if 'last_error' not in columns: connection.execute("ALTER TABLE employer_reply_drafts ADD COLUMN last_error TEXT")
    connection.commit()
...
def mark_reply_sent(connection,message_id: str,*,now=None) -> None:
    stamp=(now or datetime.now(timezone.utc)).isoformat(); connection.execute("UPDATE employer_reply_drafts SET status='SENT',sent_at=?,last_error=NULL WHERE message_id=? AND status='APPROVED'",(stamp,message_id)); connection.commit()


def mark_reply_send_failed(connection,message_id: str,error: str) -> None:
    connection.execute("UPDATE employer_reply_drafts SET last_error=? WHERE message_id=? AND status='APPROVED'",(str(error)[:500],message_id)); connection.commit()
