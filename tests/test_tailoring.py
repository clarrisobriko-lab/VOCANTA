...
class TailoringTests(unittest.TestCase):
    def profile(self, root: Path) -> ApplicantProfile:
        resume, cover, cert = root / "master.docx", root / "cover.docx", root / "cert.pdf"
        resume.write_bytes(b"docx"); cover.write_bytes(b"docx"); cert.write_bytes(b"pdf")
        return ApplicantProfile(first_name="Alex", middle_name="Morgan", last_name="Taylor", email="alex@example.com", phone="+000000000000", city="Abuja", country="Nigeria", address="", postal_code="", linkedin_url="", website_url="", work_authorization="Requires sponsorship", requires_sponsorship=True, notice_period="Immediate", salary_expectation="", resume_path=str(resume), cover_letter_path=str(cover), supporting_document_path=str(cert))

    def test_category_detection(self):
        self.assertEqual(classify_job(Job("Example", "Executive Assistant", "Remote", "test", "https://example.com")), "EXECUTIVE_OPERATIONS")
    ...
    def test_semantic_keyword_synonyms_are_normalised(self):
        cases = (("Manage complex executive diaries and coordinate senior leadership meetings", ("executive support", "calendar management", "scheduling")), ("Own talent acquisition, new hire orientation and workplace relations", ("recruitment", "onboarding", "employee relations")), ("Perform contract review, regulatory compliance and legal analysis", ("contract management", "compliance", "legal research")), ("Prepare management reports using Microsoft 365 and G Suite", ("reporting", "microsoft office", "google workspace")))
    ...
    def test_documents_are_created(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); profile = self.profile(root); job = Job("Example", "Executive Assistant", "Remote", "test", "https://example.com", description="Executive support and scheduling")
            ...
            try:
                ...
                self.assertTrue(docs.resume_path.is_file()); self.assertTrue(docs.cover_letter_path.is_file()); self.assertTrue(docs.certificate_path.is_file())
            finally: module.TAILORED_APPLICATIONS_DIR = old

    def test_unsupported_requirement_never_enters_generated_documents(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); profile = self.profile(root)
            job = Job("Example", "Executive Assistant", "Remote", "test", "https://example.com", description="Manage executive diaries and Salesforce CRM records")
            import automation.tailoring as module
            old = module.TAILORED_APPLICATIONS_DIR; module.TAILORED_APPLICATIONS_DIR = root / "out"
            try:
    ...
    def test_cv_headline_matches_job_category(self):
        cases = (("Executive Assistant", "Executive support and scheduling", "EXECUTIVE_OPERATIONS"), ("Human Resources Manager", "Recruitment, onboarding and employee relations", "HR_PEOPLE"), ("Legal Counsel", "Legal research, contracts and compliance", "LEGAL_COMPLIANCE"), ("Programme Officer", "Humanitarian nonprofit programme coordination", "NGO_PROGRAMME"))
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); profile = self.profile(root)
            import automation.tailoring as module
            old = module.TAILORED_APPLICATIONS_DIR; module.TAILORED_APPLICATIONS_DIR = root / "out"
