...
def build_html(data,due_rows,follow_stats=None,actions=None,drafts=None):
    ...
    draft_cards="".join(f"<article><h3>Approval required: {escape(d['company'])}</h3><p><strong>{escape(d['subject'])}</strong></p><pre>{escape(d['body'])}</pre><p>Message {escape(d['message_id'])}</p></article>" for d in drafts)
    ...
    ANALYTICS_FILE.write_text(f"""<!doctype html><html lang="en"><head><meta charset="utf8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{APP_DISPLAY_NAME} Analytics</title><style>body{{font-family:Arial,sans-serif;max-width:1050px;margin:30px auto;padding:0 18px;background:#f5f7fb;color:#111827}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px}}.card,article{{background:white;border:1px solid #dbe1ea;border-radius:12px;padding:18px;margin:10px 0}}.value{{font-size:30px;font-weight:700}}pre{{white-space:pre-wrap;font-family:inherit}}</style></head><body><h1>{APP_DISPLAY_NAME} Pipeline Analytics</h1><div class="grid"><div class="card"><div class="value">{data['shortlisted']}</div><div>Shortlisted</div></div><div class="card"><div class="value">{data['applied']}</div><div>Applications</div></div><div class="card"><div class="value">{data['interviews']}</div><div>Interviews</div></div><div class="card"><div class="value">{data['offers']}</div><div>Offers</div></div><div class="card"><div class="value">{len(drafts)}</div><div>Replies awaiting approval</div></div>{health}</div><section><h2>Reply approvals</h2>{draft_cards or '<p>No replies awaiting approval.</p>'}</section><section><h2>Employer actions</h2>{action_cards or '<p>No employer actions need attention.</p>'}</section><section><h2>Follow ups due</h2>{follow_cards or '<p>No follow ups are due.</p>'}</section></body></html>""",encoding="utf8")
