...
def delivered_replies(connection,limit=25):
    try: rows=connection.execute("SELECT d.message_id,d.subject,d.sent_at,d.gmail_sent_message_id,j.company,j.title FROM employer_reply_drafts d JOIN jobs j ON j.id=d.job_id WHERE d.status='SENT' AND d.archived_at IS NULL ORDER BY d.sent_at DESC LIMIT ?",(limit,)).fetchall()
    except Exception: return []
