...
def retention_anomaly_status(connection,limit=RETENTION_ANOMALY_BASELINE_RUNS,min_runs=RETENTION_ANOMALY_MIN_RUNS):
    ...
    if not values: return {'status':'unavailable','anomalous':False,'latest_removed':0,'baseline_runs':0,'baseline_mean':0.0,'baseline_deviation':0.0,'anomaly_threshold':0.0}
    latest=values[0]; baseline_values=values[1:1+max(1,int(limit))]
    if not baseline_values: return {'status':'warming','anomalous':False,'latest_removed':latest,'baseline_runs':0,'baseline_mean':0.0,'baseline_deviation':0.0,'anomaly_threshold':0.0}
    average=sum(baseline_values)/len(baseline_values); deviation=(sum((value-average)**2 for value in baseline_values)/len(baseline_values))**0.5 if len(baseline_values)>1 else 0.0; threshold=average+RETENTION_ANOMALY_SIGMA*deviation; enough=len(baseline_values)>=max(2,int(min_runs)); anomalous=enough and latest>threshold and latest>average
    return {'status':'anomaly' if anomalous else ('normal' if enough else 'warming'),'anomalous':anomalous,'latest_removed':latest,'baseline_runs':len(baseline_values),'baseline_mean':round(average,2),'baseline_deviation':round(deviation,2),'anomaly_threshold':round(threshold,2)}
...
def build_query_html(scope,query,items,meta,retention=None,trend=None,anomaly=None):
    retention=retention or {'archived_replies':0,'audit_events':0,'created_at':''}; trend=trend or {'runs':0,'total_removed':0,'average_removed':0,'peak_removed':0,'latest_removed':0,'direction':'flat'}; anomaly=anomaly or {'status':'unavailable','latest_removed':0,'baseline_runs':0,'baseline_mean':0,'baseline_deviation':0,'anomaly_threshold':0}; q=escape(query,quote=True); scope=scope if scope in {'pending','approved','delivered','archived','audit'} else 'audit'; rows=[]
    for item in items:
