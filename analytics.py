...
def paginate_records(records,page=1,page_size=PAGE_SIZE):
    page_size=max(1,int(page_size)); total=len(records); pages=max(1,(total+page_size-1)//page_size); page=min(max(1,int(page)),pages); start=(page-1)*page_size
    return records[start:start+page_size],{'page':page,'pages':pages,'total':total,'page_size':page_size}
...
def delivered_replies(connection,limit=500,archived=False):
...
def reply_audit(connection,limit=500):
...
def build_html(data,due_rows,follow_stats=None,actions=None,pending=None,approved=None,audit=None,delivered=None,archived=None):
    ...
    def pager(scope): return f"<div class='pager' data-pager='{scope}'><button type='button' data-dir='prev'>Previous</button><span></span><button type='button' data-dir='next'>Next</button></div>"
    ...
    ANALYTICS_FILE.write_text(f"""<!doctype html><html><head><meta charset="utf8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{APP_DISPLAY_NAME} Analytics</title><style>body{{font-family:Arial,sans-serif;max-width:1050px;margin:30px auto;padding:0 18px;background:#f5f7fb;color:#111827}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px}}.card,article{{background:white;border:1px solid #dbe1ea;border-radius:12px;padding:18px;margin:10px 0}}.value{{font-size:30px;font-weight:700}}pre{{white-space:pre-wrap;font-family:inherit}}label{{display:block;font-weight:700;margin-top:10px}}input,textarea,select{{width:100%;box-sizing:border-box;padding:10px;margin:6px 0 12px;border:1px solid #cbd5e1;border-radius:8px;font:inherit}}button{{padding:10px 16px;font-weight:700;cursor:pointer;margin-right:8px}}button:disabled{{cursor:default;opacity:.45}}table{{width:100%;border-collapse:collapse;background:white}}th,td{{padding:10px;border:1px solid #dbe1ea;text-align:left;vertical-align:top}}code{{font-size:12px}}.filters{{display:grid;grid-template-columns:2fr 1fr;gap:12px;background:white;padding:14px;border-radius:12px;margin:18px 0}}.pager{{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin:12px 0}}.pager span{{min-width:130px;text-align:center}}</style></head><body><h1>{APP_DISPLAY_NAME} Pipeline Analytics</h1><div class="filters"><input id="search" type="search" placeholder="Search company, role, subject, event, Gmail ID"><select id="scope"><option value="all">All reply records</option><option value="pending">Awaiting approval</option><option value="approved">Approved to send</option><option value="delivered">Delivered</option><option value="archived">Archived</option><option value="audit">Audit history</option></select></div><div class="grid"><div class="card"><div class="value">{data['shortlisted']}</div><div>Shortlisted</div></div><div class="card"><div class="value">{data['applied']}</div><div>Applications</div></div><div class="card"><div class="value">{data['interviews']}</div><div>Interviews</div></div><div class="card"><div class="value">{data['offers']}</div><div>Offers</div></div><div class="card"><div class="value">{len(pending)}</div><div>Awaiting approval</div></div><div class="card"><div class="value">{len(approved)}</div><div>Approved to send</div></div><div class="card"><div class="value">{len(delivered)}</div><div>Delivered replies</div></div><div class="card"><div class="value">{len(archived)}</div><div>Archived replies</div></div>{health}</div><section data-scope="pending"><h2>Awaiting approval</h2>{pending_cards or '<p>None.</p>'}{pager('pending')}</section><section data-scope="approved"><h2>Approved to send</h2>{approved_cards or '<p>None.</p>'}{pager('approved')}</section><section data-scope="delivered"><h2>Delivered replies</h2><table><thead><tr><th>Sent</th><th>Company</th><th>Role</th><th>Subject</th><th>Status</th><th>Gmail message ID</th><th>Action</th></tr></thead><tbody>{delivered_rows or '<tr><td colspan="7">No delivered replies yet.</td></tr>'}</tbody></table>{pager('delivered')}</section><section data-scope="archived"><h2>Archived replies</h2><table><thead><tr><th>Archived</th><th>Company</th><th>Role</th><th>Subject</th><th>Gmail message ID</th><th>Action</th></tr></thead><tbody>{archived_rows or '<tr><td colspan="6">No archived replies.</td></tr>'}</tbody></table>{pager('archived')}</section><section data-scope="audit"><h2>Reply history</h2><table><thead><tr><th>Time</th><th>Company</th><th>Role</th><th>Event</th><th>Detail</th></tr></thead><tbody>{audit_rows or '<tr><td colspan="5">No reply activity yet.</td></tr>'}</tbody></table>{pager('audit')}</section><section><h2>Employer actions</h2>{action_cards or '<p>None.</p>'}</section><section><h2>Follow ups due</h2>{follow_cards or '<p>None.</p>'}</section><script>const q=document.getElementById('search'),s=document.getElementById('scope'),size={PAGE_SIZE},pages={{}};function render(section){{const scope=section.dataset.scope,term=q.value.trim().toLowerCase(),items=[...section.querySelectorAll('.searchable')],matches=items.filter(item=>!term||item.dataset.search.includes(term)),total=Math.max(1,Math.ceil(matches.length/size));pages[scope]=Math.min(Math.max(1,pages[scope]||1),total);const start=(pages[scope]-1)*size;items.forEach(item=>item.style.display='none');matches.slice(start,start+size).forEach(item=>item.style.display='');const pager=section.querySelector('.pager');if(pager){{pager.querySelector('span').textContent=`Page ${{pages[scope]}} of ${{total}} · ${{matches.length}} records`;pager.querySelector('[data-dir="prev"]').disabled=pages[scope]<=1;pager.querySelector('[data-dir="next"]').disabled=pages[scope]>=total}}}}function apply(reset=false){{const scope=s.value;document.querySelectorAll('section[data-scope]').forEach(section=>{{if(reset) pages[section.dataset.scope]=1;section.style.display=scope==='all'||section.dataset.scope===scope?'':'none';render(section)}})}}document.querySelectorAll('.pager button').forEach(button=>button.addEventListener('click',()=>{{const section=button.closest('section'),scope=section.dataset.scope,dir=button.dataset.dir;pages[scope]=(pages[scope]||1)+(dir==='next'?1:-1);render(section)}}));q.addEventListener('input',()=>apply(true));s.addEventListener('change',()=>apply(false));apply(true);</script></body></html>""",encoding='utf8')
