...
def build_html(data,due_rows,follow_stats=None,actions=None,pending=None,approved=None,audit=None):
    EXPORT_DIR.mkdir(parents=True,exist_ok=True); follow_stats=follow_stats or {}; actions=actions or []; pending=pending or []; approved=approved or []; audit=audit or []
    ...
    ANALYTICS_FILE.write_text(f"""<!doctype html><html><head><meta charset="utf8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{APP_DISPLAY_NAME} Analytics</title><style>body{{font-family:Arial,sans-serif;max-width:1050px;margin:30px auto;padding:0 18px;background:#f5f7fb;color:#111827}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px}}.card,article{{background:white;border:1px solid #dbe1ea;border-radius:12px;padding:18px;margin:10px 0}}.value{{font-size:30px;font-weight:700}}pre{{white-space:pre-wrap;font-family:inherit}}label{{display:block;font-weight:700;margin-top:10px}}input,textarea{{width:100%;box-sizing:border-box;padding:10px;margin:6px 0 12px;border:1px solid #cbd5e1;border-radius:8px;font:inherit}}button{{padding:10px 16px;font-weight:700;cursor:pointer;margin-right:8px}}table{{width:100%;border-collapse:collapse;background:white}}th,td{{padding:10px;border:1px solid #dbe1ea;text-align:left;vertical-align:top}}</style></head><body><h1>{APP_DISPLAY_NAME} Pipeline Analytics</h1><div class="grid"><div class="card"><div class="value">{data['shortlisted']}</div><div>Shortlisted</div></div><div class="card"><div class="value">{data['applied']}</div><div>Applications</div></div><div class="card"><div class="value">{data['interviews']}</div><div>Interviews</div></div><div class="card"><div class="value">{data['offers']}</div><div>Offers</div></div><div class="card"><div class="value">{len(pending)}</div><div>Awaiting approval</div></div><div class="card"><div class="value">{len(approved)}</div><div>Approved to send</div></div>{health}</div><section><h2>Awaiting approval</h2>{pending_cards or '<p>None.</p>'}</section><section><h2>Approved to send</h2>{approved_cards or '<p>None.</p>'}</section><section><h2>Reply history</h2><table><thead><tr><th>Time</th><th>Company</th><th>Role</th><th>Event</th><th>Detail</th></tr></thead><tbody>{audit_rows or '<tr><td colspan="5">No reply activity yet.</td></tr>'}</tbody></table></section><section><h2>Employer actions</h2>{action_cards or '<p>None.</p>'}</section><section><h2>Follow ups due</h2>{follow_cards or '<p>None.</p>'}</section></body></html>""",encoding='utf8')


def main():
    console=Console(); database=Database()
    try:
        data=database.analytics(SHORTLIST_SCORE); due=database.list_due_follow_ups(); stats=follow_up_statistics(database.connection); actions=response_actions(database.connection); pending=reply_drafts(database.connection,'AWAITING_APPROVAL'); approved=reply_drafts(database.connection,'APPROVED'); audit=reply_audit(database.connection)
        table=Table(title=f"{APP_DISPLAY_NAME} Pipeline Analytics"); table.add_column('Metric'); table.add_column('Value',justify='right')
        for label,value in [('Shortlisted',data['shortlisted']),('Applications',data['applied']),('Interviews',data['interviews']),('Offers',data['offers']),('Awaiting approval',len(pending)),('Approved to send',len(approved)),('Reply history',len(audit))]: table.add_row(label,str(value))
        console.print(table); build_html(data,due,stats,actions,pending,approved,audit); return 0
