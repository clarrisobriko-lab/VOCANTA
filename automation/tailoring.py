...
EXPERIENCE_PRIORITY_TERMS = {"EXECUTIVE_OPERATIONS": ("executive", "calendar", "meeting", "scheduling", "administrative", "reporting", "communication", "coordination"), "HR_PEOPLE": ("human resource", "hr personnel", "recruitment", "onboarding", "employee", "workforce", "personnel", "training"), "LEGAL_COMPLIANCE": ("legal", "counsel", "contracts", "case", "compliance", "policy", "research", "client"), "NGO_PROGRAMME": ("ngo", "human rights", "government", "legal defence", "access to justice", "programme", "stakeholder", "client")}
EXPERIENCE_TITLE_TERMS = {"EXECUTIVE_OPERATIONS": ("executive", "administrative", "hr personnel"), "HR_PEOPLE": ("human resource", "hr personnel"), "LEGAL_COMPLIANCE": ("legal", "counsel"), "NGO_PROGRAMME": ("legal defence", "programme", "ngo")}
...
def prioritize_experience(category: str):
    ...
    def relevance(item) -> int:
        ...
        return sum(3 for term in title_terms if term in title_text) + sum(body_text.count(term) for term in terms) + sum(title_text.count(term) for term in terms)
...
def build_tailored_cv(job: Job, profile: ApplicantProfile, output: Path) -> None:
    category = classify_job(job); keywords = extract_keywords(job); document = Document(); _add_contact(document, profile)
...
def build_tailored_cover_letter(job: Job, profile: ApplicantProfile, output: Path) -> None:
    category = classify_job(job); keywords = extract_keywords(job); document = Document(); _add_contact(document, profile)
    document.add_paragraph(date.today().strftime("%d %B %Y")); document.add_paragraph("Dear Hiring Team,"); document.add_paragraph(f"I am writing to apply for the {job.title} position at {job.company}. {CATEGORY_SUMMARIES[category]}")
...
def tailor_documents(job: Job, job_id: int, profile: ApplicantProfile) -> TailoredDocuments:
    ...
    resume = folder / "tailored_cv.docx"; cover = folder / "tailored_cover_letter.docx"; build_tailored_cv(job, profile, resume); build_tailored_cover_letter(job, profile, cover); certificate = None
