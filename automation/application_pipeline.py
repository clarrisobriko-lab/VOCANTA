...
_CONFIRMED_STATUSES={"SUBMITTED","SUCCESS","AUTO_SUBMITTED","CONFIRMED"};_AMBIGUOUS_STATUSES={"UNKNOWN","SUBMISSION_UNVERIFIED"};_HUMAN_STATUSES={"HUMAN_REQUIRED","HUMAN_VERIFICATION","MANUAL_REQUIRED","READY_TO_REVIEW"}
...
def _apply_with_recovery(job,job_id,profile,browser_engine_factory,*,max_attempts=3,sleep_fn=time.sleep):
    last=None;job_context="\n".join(x for x in (job.title,job.description) if x)
...
def _persist_pipeline_evidence(job,job_id,package,automation,*,database=None,application_run_id=None):
    ...
    if database is not None:
        connection=getattr(database,"connection",database);record_submission_evidence(connection,job_id=job_id,application_run_id=application_run_id,evidence_path=path,package_sha256=evidence.package_sha256,ats=evidence.ats,outcome=evidence.outcome,confirmation_url=evidence.confirmation_url,screenshot_path=evidence.screenshot_path);connection.commit()
    return path
def run_application_pipeline(job:Job,job_id:int,profile:ApplicantProfile,*,scorer:Scorer|None=None,browser_engine_factory=HardenedBrowserApplicationEngine,database=None,application_run_id:int|None=None)->PipelineResult:
    ...
    documents=tailor_documents(job,job_id,profile);package=build_application_package(job,documents,decision);browser_profile=profile_for_package(profile,package);validate_browser_documents(browser_profile);automation=_apply_with_recovery(job,job_id,browser_profile,browser_engine_factory);evidence_path=_persist_pipeline_evidence(job,job_id,package,automation,database=database,application_run_id=application_run_id);return PipelineResult(decision,documents,package,automation,evidence_path)
