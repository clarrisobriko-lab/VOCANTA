...
from core.response_notification_store import claim_notification, mark_notification_delivered, mark_notification_failed
...
def notify_processed_responses(connection, results, sender) -> list[str]:
    ...
    for result in results:
        if result.status not in {'PROCESSED','DUPLICATE'} or result.job_id is None: continue
        ...
        response=EmployerResponse(str(row[0]),int(row[1]),str(row[2])); note=build_response_notification(result.message_id,str(row[4]),str(row[5]),str(row[3]),response)
        if note is None or not claim_notification(connection,note.message_id,note.priority): continue
        try: sender.send(note.subject,note.body)
        except Exception as exc:
            mark_notification_failed(connection,note.message_id,str(exc)); continue
