...
def identify_field(label):
    ...
    return {
        ...
        Intent.MIDDLE_NAME: "middle_name",
        Intent.LAST_NAME: "last_name",
        Intent.FULL_NAME: "full_name",
        Intent.EMAIL: "email",
        Intent.PHONE: "phone",
        Intent.CITY: "city",
        Intent.CURRENT_COUNTRY: "country",
        Intent.ADDRESS: "address",
        Intent.POSTAL_CODE: "postal_code",
        Intent.LINKEDIN: "linkedin_url",
        Intent.WEBSITE: "website_url",
        Intent.NOTICE_PERIOD: "notice_period",
...
def _label_for(locator):
    ...
    try:
        ...
        pass
    try:
        return locator.locator("xpath=../..").inner_text()[:900]
    except Exception:
        return ""
...
def _has_middle_name_field(frame):
    ...
    for i in range(controls.count()):
        try:
            if identify_field(_label_for(controls.nth(i))) == "middle_name":
                return True
        except Exception:
            continue
    return False
...
def _structured_answer(label, resolver):
...
def _fill_text_inputs(frame, profile, job_context="", resolver=None):
    filled = 0
    audits = []
    resolver = resolver or FormProfileResolver(profile)
    ...
    for i in range(controls.count()):
        ...
        try:
            ...
            if not value:
                value, source = _structured_answer(label, resolver)
            if not value:
            ...
            if not value:
                continue
            value = sanitize_applicant_text(value)
            control.fill(value)
            filled += 1
            audits.append(
                FieldAudit(
                    label[:180],
                    identify_intent(label).value,
                    _is_required(control),
                    "filled",
                    source or "cv semantic evidence",
                )
            )
...
def _upload_documents(frame, profile):
    ...
    for i in range(files.count()):
        c = files.nth(i)
        label = normalize(_label_for(c))
        try:
            if any(t in label for t in ("cover", "letter")) and profile.cover_letter_path:
                c.set_input_files(profile.cover_letter_path)
                uploaded += 1
                cover = True
                ...
                c.set_input_files(profile.supporting_document_path)
                uploaded += 1
            elif profile.resume_path:
                c.set_input_files(profile.resume_path)
                uploaded += 1
                cv = True
        except Exception:
            continue
...
def _preferred_answer(label, profile, job_context="", resolver=None):
    if resolver:
        value, source = _structured_answer(label, resolver)
        if value:
            return value, source
    ...
    if resolution.value and resolution.auto_fill_allowed:
        return resolution.value, resolution.reason or f"profile.{resolution.intent.value.lower()}"
    grounded = answer_application_question(label, profile, job_context=job_context)
    return (grounded.value, grounded.source) if grounded else ("", "")


def _select_common_options(frame, profile, job_context="", resolver=None):
    filled = 0
    audits = []
    resolver = resolver or FormProfileResolver(profile)
    ...
    for i in range(selects.count()):
        s = selects.nth(i)
        try:
            if not s.is_visible() or s.is_disabled():
                continue
            current = normalize(s.input_value())
            if current and current not in {"select", "choose", "please select"}:
                continue
            label = _label_for(s)
            preferred, source = _preferred_answer(label, profile, job_context, resolver)
            if preferred and _select_by_visible_text(s, preferred):
                filled += 1
                audits.append(FieldAudit(label[:180], identify_intent(label).value, _is_required(s), "filled", source))
        except Exception:
            continue
    return filled, audits


def _fill_custom_comboboxes(frame, profile, job_context="", resolver=None):
    filled = 0
    audits = []
    resolver = resolver or FormProfileResolver(profile)
    ...
    for i in range(boxes.count()):
        ...
        try:
            ...
            if not preferred:
                continue
            box.click()
            box.page.wait_for_timeout(150)
            option = frame.get_by_role("option", name=preferred, exact=True)
            if not option.count():
                option = frame.get_by_role("option", name=preferred, exact=False)
            if option.count():
                option.first.click()
                filled += 1
                audits.append(FieldAudit(label[:180], identify_intent(label).value, _is_required(box), "filled", source))
        except Exception:
            continue
    return filled, audits
...
def _required_unanswered(frame):
    unanswered = []
    controls = frame.locator(
        'input[required]:not([type="hidden"]), textarea[required], select[required], [role="combobox"][aria-required="true"], input[aria-required="true"]:not([type="hidden"]), textarea[aria-required="true"], select[aria-required="true"]'
    )
    for i in range(controls.count()):
        c = controls.nth(i)
        try:
            if not c.is_visible() or c.is_disabled():
                continue
            tag = c.evaluate("(node)=>node.tagName.toLowerCase()")
            typ = normalize(c.get_attribute("type"))
            answered = True
            if tag == "select":
                answered = bool(c.input_value())
            elif c.get_attribute("role") == "combobox":
                answered = normalize(c.get_attribute("aria-expanded")) != "" and normalize(c.inner_text()) not in {"", "select", "choose"}
            elif typ in {"checkbox", "radio"}:
                name = c.get_attribute("name")
                answered = frame.locator(f'input[name="{name}"]:checked').count() > 0 if name else c.is_checked()
            elif typ == "file":
                answered = bool(c.input_value())
            else:
                answered = bool(c.input_value().strip())
            if not answered:
                unanswered.append(" ".join((_label_for(c) or c.get_attribute("name") or "Unknown field").split())[:180])
        except Exception:
            continue
    return unanswered
    ...
    for fi, frame in enumerate(form_frames(page)):
        ...
        for i in range(controls.count()):
            ...
            try:
                ...
                negative = bool(text and any(t in text for t in NEGATIVE_BUTTON_TEXTS))
                matched = bool(text) and (
                    any(text == t for t in texts) if exact else any(text == t or t in text for t in texts)
                )
                entries.append(
                    f"frame={fi} index={i} text={text!r} visible={visible} enabled={enabled} negative={negative} matched={matched}"
                )
            except Exception as exc:
                entries.append(f"frame={fi} index={i} inspection_error={type(exc).__name__}: {exc}")
        ...
        for i in range(controls.count()):
            ...
            try:
                if not c.is_visible() or not c.is_enabled():
                    continue
                text = _control_text(c)
                if not text or any(t in text for t in NEGATIVE_BUTTON_TEXTS):
                    continue
                if any(text == t for t in texts) if exact else any(text == t or t in text for t in texts):
                    return frame, c
            except Exception:
                continue
    return None
...
def _is_required(c):
    return bool(c.get_attribute("required") is not None or normalize(c.get_attribute("aria-required")) == "true")
...
def _choose_radio_or_checkbox(frame, profile, job_context=""):
    ...
    for i in range(controls.count()):
        c = controls.nth(i)
        try:
            if not c.is_visible() or c.is_disabled() or c.is_checked():
                continue
            ...
            groups.add(name)
            label = _label_for(c)
            group = frame.locator(f'input[name="{name}"]') if not name.startswith("__single_") else c
            combined = label
            if group.count() > 1:
                try:
                    combined = group.first.locator("xpath=../..").inner_text()[:900]
                except Exception:
                    pass
            desired, source = _preferred_answer(combined, profile, job_context)
            ...
            for oi in range(group.count()):
                option = group.nth(oi)
                option_label = normalize(_label_for(option))
                option_value = normalize(option.get_attribute("value"))
                if desired == option_label or desired == option_value or desired in option_label:
                    chosen = option
                    break
            if chosen is not None:
                ...
                audits.append(
                    FieldAudit(
                        combined[:180],
                        identify_intent(combined).value,
                        _is_required(c),
                        "filled",
                        source or "grounded semantic answer",
                    )
                )
        except Exception:
            continue
...
def fill_application_form(
    ...
    *,
    ...
    for frame in form_frames(page):
        ...
        detected += frame.locator('input, textarea, select, [role="combobox"]').count()
        required += frame.locator('[required], [aria-required="true"]').count()
        n, a = _fill_text_inputs(frame, profile, job_context, resolver)
        filled += n
        audits.extend(a)
        n, cv, cover = _upload_documents(frame, profile)
        filled += n
        cv_uploaded = cv_uploaded or cv
        cover_uploaded = cover_uploaded or cover
        n, a = _select_common_options(frame, profile, job_context, resolver)
        filled += n
        audits.extend(a)
        n, a = _fill_custom_comboboxes(frame, profile, job_context, resolver)
        filled += n
        audits.extend(a)
        n, a = _choose_radio_or_checkbox(frame, profile, job_context)
        filled += n
        audits.extend(a)
        unanswered.extend(_required_unanswered(frame))
    submit = find_action_control(page, final_submit_texts, exact=True)
    next_step = find_action_control(page, NEXT_STEP_TEXTS)
    return FillResult(
        filled,
        tuple(dict.fromkeys(unanswered)),
        submit is not None,
        next_step is not None,
        audit_action_controls(page, final_submit_texts, exact=True),
        (),
        detected,
        required,
