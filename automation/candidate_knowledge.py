...
class CandidateKnowledge:
    ...
    def rank(self, question: str, job_context: str = "", *, limit: int = 6) -> tuple[EvidenceChunk, ...]:
        q = _tokens(question); j = _tokens(job_context); ranked: list[EvidenceChunk] = []
        for chunk in self.chunks:
            tokens = _tokens(chunk.text)
            if not tokens: continue
            direct = len(q & tokens); vacancy = len(j & tokens)
            phrase_bonus = sum(2 for phrase in _phrases(question) if phrase in chunk.text.lower())
            score = direct * 4 + vacancy * 1.5 + phrase_bonus
            if score > 0: ranked.append(EvidenceChunk(chunk.text, chunk.source, score))
    ...
    def narrative(self, question: str, job_context: str = "", *, max_chars: int = 1400) -> str:
        ...
        sentences: list[str] = []; seen: set[str] = set()
        for item in ranked:
            for sentence in re.split(r"(?<=[.!?])\s+", item.text):
                cleaned = sanitize_applicant_text(sentence).strip(" .")
                key = cleaned.lower()
                if not cleaned or key in seen: continue
                seen.add(key)
                sentences.append(cleaned.rstrip(".") + ".")
                if len(" ".join(sentences)) >= max_chars: break
            if len(" ".join(sentences)) >= max_chars: break
        return " ".join(sentences)[:max_chars].strip()


def _tokens(value: str) -> set[str]:
    return {token for token in re.findall(r"[a-z0-9]+", (value or "").lower()) if len(token) > 2 and token not in STOP_WORDS}


def _phrases(value: str) -> tuple[str, ...]:
    text = " ".join((value or "").lower().split())
    phrases = ("executive support", "calendar management", "administrative support", "stakeholder management", "travel management", "high accuracy", "employee relations", "legal research", "records management", "project coordination", "client communication", "internal communications", "case management")
    return tuple(p for p in phrases if p in text)


def _docx_chunks(path: Path) -> list[EvidenceChunk]:
    if not path.is_file() or path.suffix.lower() != ".docx": return []
    try: document = Document(path)
    except Exception: return []
    chunks: list[EvidenceChunk] = []
    for index, paragraph in enumerate(document.paragraphs):
        text = sanitize_applicant_text(paragraph.text)
        if len(text) >= 20: chunks.append(EvidenceChunk(text, f"cv.paragraph.{index}"))
    for table_index, table in enumerate(document.tables):
        for row_index, row in enumerate(table.rows):
            text = sanitize_applicant_text(" | ".join(cell.text.strip() for cell in row.cells if cell.text.strip()))
            if len(text) >= 20: chunks.append(EvidenceChunk(text, f"cv.table.{table_index}.{row_index}"))
    return chunks


def _profile_chunks(profile: ApplicantProfile) -> list[EvidenceChunk]:
    chunks: list[EvidenceChunk] = []; education = profile.highest_education
    chunks.append(EvidenceChunk(sanitize_applicant_text(f"{education.degree}, {education.discipline}, {education.institution}, {education.graduation_year}, {education.country}"), "profile.education"))
    for index, item in enumerate(profile.employment_history):
        ...
        if item.summary: text += f". {item.summary}"
        if text: chunks.append(EvidenceChunk(sanitize_applicant_text(text), f"profile.employment.{index}"))
    for question, answer in profile.standard_answers.items():
        if answer: chunks.append(EvidenceChunk(sanitize_applicant_text(answer), f"profile.standard_answers.{question}"))
    return chunks


def load_candidate_knowledge(profile: ApplicantProfile) -> CandidateKnowledge:
    source = getattr(profile, "knowledge_resume_path", "") or getattr(profile, "source_resume_path", "") or profile.resume_path
    cv_chunks = _docx_chunks(Path(source).expanduser())
