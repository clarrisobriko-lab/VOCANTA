...
def build_gmail_service(*,token_path: str="gmail_token.json",credentials_path: str="gmail_credentials.json"):
    ...
    credentials=None; token=Path(token_path)
    ...
    if not credentials or not credentials.valid:
        ...
        flow=InstalledAppFlow.from_client_secrets_file(str(source),SCOPES); credentials=flow.run_local_server(port=0); token.write_text(credentials.to_json(),encoding="utf8")
