...
class GmailInboxSource:
    ...
    def fetch_recent(self, *, query: str = "in:inbox newer_than:30d", limit: int = 100) -> list[dict]:
        ...
        for item in response.get("messages",[]):
            ...
            messages.append(self._normalize(raw))
        return messages

    @staticmethod
    def _normalize(raw: dict) -> dict:
        payload=raw.get("payload",{}); headers={h.get("name","").lower():h.get("value","") for h in payload.get("headers",[])}
        body=GmailInboxSource._body(payload) or raw.get("snippet","")
        return {"id":raw.get("id",""),"from":parseaddr(headers.get("from",""))[1],"subject":headers.get("subject",""),"body":body,"date":headers.get("date","")}
