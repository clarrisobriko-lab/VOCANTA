...
from core.employer_reply_store import ensure_reply_schema, mark_reply_send_failed, mark_reply_sent, record_reply_event
...
def send_approved_reply(connection,message_id: str,recipient: str | None,sender) -> bool:
    ensure_reply_schema(connection); row=connection.execute("SELECT subject,body,status FROM employer_reply_drafts WHERE message_id=?",(message_id,)).fetchone()
